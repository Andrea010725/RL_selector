...
class CARLABaseEnvironment(gym.Env):
    ...
    def __init__(self, address='localhost', port=2000, timeout=150.0, image_shape=(150, 200, 3), window_size=(800, 600),
                 vehicle_filter='vehicle.tesla.model3', fps=10.0, render=True, debug=True, spawn: dict = None,
                 ignore_traffic_light=True, path: dict = None, town: str = None,
                 weather=carla.WeatherParameters.ClearNoon, skip_frames=10):
        """Arguments:
            - path: dict =
        ...
        # Map
        self.current_town = 'Town02'  # loaded by default
    ...
    #         if self.ood_flag is not True :
    #             ipdb.set_trace()
    #             self.spawn_actors_debug(self.origin_location, self.spawn_dict, self.selected_task)   #   这里进去生成 车和行人
    #         else:
    #             self.spawn_actors_OOD(self.origin_location, self.spawn_dict, self.selected_task)
    ...
    def reset(self) -> dict:
        ...
        # ipdb.set_trace()
        # self.spawn_actors_debug(self.origin_location, self.spawn_dict, self.selected_task)   #   这里进去生成 车和行人
        self.spawn_actors_OOD(self.origin_location, self.spawn_dict, self.selected_task)  # 这里进去生成 车和行人
    ...
    def spawn_actors_debug(self, origin_location: carla.Location, spawn_dict: dict, selected_task: str, hybrid=True,
                           safe=True):
        """Instantiate vehicles and pedestrians in the current world"""
        # print('selected_task',selected_task)
        origin = carla.Location(x=origin_location.x, y=origin_location.y, z=origin_location.z)
        ego_wp = self.world.get_map().get_waypoint(location=origin, project_to_road=True,
        ...
        if task == 'FrontBrake':
            self.special_scene = env_utils.FrontBrake(self.world, ego_wp)
        ...
        if task == 'ObstacleAhead':
            self.special_scene = env_utils.ObstacleAhead(self.world, ego_wp)

        if task == 'Trilemma':
            self.special_scene = env_utils.Trilemma(self.world, ego_wp)
    ...
    def spawn_actors_OOD(self, origin_location: carla.Location, spawn_dict: dict, selected_task: str, hybrid=True,
                         safe=True):
        """Instantiate vehicles and pedestrians in the current world"""
        print('selected_task', selected_task)
        origin = carla.Location(x=origin_location.x, y=origin_location.y, z=origin_location.z)
        ...
        if task == 'JunctionStright':
            self.special_scene = env_utils.JunctionStright(self.world, ego_wp)

        if task == 'Trilemma':
            self.special_scene = env_utils.Trilemma(self.world, ego_wp)

    def destroy_actors(self):
        """Removes the previously spawned actors (vehicles and pedestrians/walkers) with improved stability"""
        import time

        try:
            # Remove vehicles one by one to avoid batch operation issues
            if self.vehicles:
                ...
                for vehicle_id in self.vehicles:
                    try:
                        ...
                        print(f"Warning: Failed to destroy vehicle {vehicle_id}: {e}")
                    time.sleep(0.01)  # Small delay between destructions
                self.vehicles.clear()

            # Handle walkers more carefully
            if self.walkers_ids:
                ...
                try:
                    ...
                    for i in range(0, len(self.walkers_ids), 2):
                        try:
                            if i < len(actors) and actors[i] and actors[i].is_alive:
                                actors[i].stop()
                        except Exception as e:
                            print(f"Warning: Failed to stop walker controller: {e}")

                    time.sleep(0.5)  # Wait for controllers to stop

                    # Destroy walkers one by one
                    for walker_id in self.walkers_ids:
                        try:
                            walker = self.world.get_actor(walker_id)
                            if walker and walker.is_alive:
                                walker.destroy()
                        except Exception as e:
                            print(f"Warning: Failed to destroy walker {walker_id}: {e}")
                        time.sleep(0.01)

                    self.walkers_ids.clear()
                except Exception as e:
                    print(f"Warning: Error during walker destruction: {e}")
                    self.walkers_ids.clear()
            ...
            print(f"Error in destroy_actors: {e}")
            # Force clear the lists even if destruction failed
            self.vehicles.clear()
            self.walkers_ids.clear()
    ...
    def step(self, actions):
        ...
        # str 待修改
        # 这里添加一个计数器 超过5000 就换reward
        # reward = self.reward(actions)

        # 新增
        #         if self.step_counter > 5000:
        #             # 使用新的reward计算方式
        #             reward = self.reward_second_step(actions)
        #             if self.step_counter > 10000:  # 重置计数器
        #                 self.step_counter = 0
        #         else:
        #         # 使用原始reward计算方式
        #             reward = self.reward_first_step(actions)
    ...
    def close(self):
        ...
        try:
            ...
            # Destroy main vehicle with error handling
            if hasattr(self, 'vehicle') and self.vehicle:
                try:
                    if self.vehicle.is_alive:
                        self.vehicle.destroy()
                except Exception as e:
                    print(f"Warning: Failed to destroy main vehicle: {e}")
                finally:
                    self.vehicle = None

            # Handle camera cleanup safely
            if hasattr(self, 'ego_cam') and self.ego_cam:
                try:
                    if hasattr(self.ego_cam, 'destroy'):
            ...
            # Exit sync mode safely
            if hasattr(self, 'sync_mode_enabled') and self.sync_mode_enabled:
                try:
                    if hasattr(self, '_ThreeCameraCARLAEnvironment__exit__'):
                        self.__exit__()
                    elif hasattr(self, '__exit__'):
                        self.__exit__(None, None, None)
                except Exception as e:
                    print(f"Warning: Failed to exit sync mode: {e}")
                finally:
                    self.sync_mode_enabled = False
            ...
            try:
                ...
                print(f"Warning: Parent close failed: {e}")
    ...
    def reset_world(self):
        ...
        if self.origin_type == 'map':
            ...
            self.origin_location = carla.Location(x=self.origin.location.x, y=self.origin.location.y,
                                                  z=self.origin.location.z)
    ...
    def choose_tasks(self):
        ...
        selected_task = 'ObstacleAhead'  # 'Trilemma'#'ObstacleAhead'#'GhostA'#
        # selected_task = 'FrontBrake'
...
class OneCameraCARLAEnvironment(CARLABaseEnvironment):
    ...
    VEHICLE_FEATURES = dict(space=spaces.Box(low=np.array([0.0, -np.inf, 0.0, -1.0, -np.inf]),
                            ...
                            default=np.zeros(shape=4, dtype=np.float32))  # 5 -- 4

    # Road: intersection (bool), junction (bool), speed_limit, traffic_light (presence + state), lane type and change,  # change
    ...
    INFO_SPACE = spaces.Dict(speed=spaces.Box(low=0.0, high=150.0, shape=(1,)),
                             ...
                             # similarity=spaces.Box(low=-1.0, high=1.0, shape=(1,)),    # change
    ...
    def __init__(self, *args, disable_reverse=False, min_throttle=0.0, camera='segmentation',
        ...
        # self.image_space = spaces.Box(low=0.0, high=1.0, shape=self.image_shape)   # change
        ...
        # Observations
        # self.default_image = np.zeros(shape=self.image_shape, dtype=np.float32)  # change
    ...
    def observation_space(self) -> spaces.Space:
        ...
        #                    past_control=self.CONTROL['space'], command=self.COMMAND_SPACE, image=self.image_space)   # change
        return spaces.Dict(vehicle=self.VEHICLE_FEATURES['space'],
                           past_control=self.CONTROL['space'], command=self.COMMAND_SPACE)
    ...
    def define_sensors(self) -> dict:
        ...
        #                                                     image_size_x=self.image_size[0],
        #                                                     image_size_y=self.image_size[1],
        #                                                     sensor_tick=self.tick_time)   # change
    ...
    def on_sensors_data(self, data: dict) -> dict:
        ...
        #             data['camera'] = env_utils.cv2_grayscale(data['camera'])   #CHANGE
    ...
    def get_observation(self, sensors_data: dict) -> dict:
        if len(sensors_data.keys()) == 0:
            ...
            return dict(vehicle=self.VEHICLE_FEATURES['default'],
                        past_control=self.CONTROL['default'],
                        command=RoadOption.VOID.to_one_hot())

        # image = sensors_data['camera']    # change
        ...
        #            command=self.next_command.to_one_hot())   # change
        obs = dict(vehicle=vehicle_obs, past_control=control_obs,
    ...
    def get_info(self) -> dict:
        ...
        return dict(speed=utils.speed(self.vehicle), speed_limit=self.vehicle.get_speed_limit(),
                    # similarity=self.similarity,   # change
                    distance_to_next_waypoint=self.route.distance_to_next_waypoint())
    ...
    def _get_vehicle_features(self):
        ...
        acceleration = env_utils.magnitude(imu_sensor.accelerometer)  # change * env_utils.sign(self.similarity)
    ...
    def _update_env_state(self):
        if self.use_planner:
            ...
            # self._update_waypoint_similarity()    # change
...
class ThreeCameraCARLAEnvironment(OneCameraCARLAEnvironment):
    ...
    def __init__(self, *args, image_shape=(120, 160, 1), window_size=(600, 300), **kwargs):
        ...
        # image_shape = (image_shape[0], image_shape[1] * 3, image_shape[2])     # CHANGE

        # super().__init__(*args, image_shape=image_shape, window_size=window_size, **kwargs) # CHANGE
        super().__init__(*args, window_size=window_size, **kwargs)
        # self.image_size = (image_shape[1] // 3, image_shape[0])    # CHANGE
