...
def dynamics_layers(inputs: dict, time_horizon: int, **kwargs):
    ...
    # image_out = nn.shufflenet_v2(inputs['state_image'], time_horizon, **kwargs.get('shufflenet', {}))  # change
    # road_out = nn.feature_net(inputs['state_road'], time_horizon, **kwargs.get('road', dict(normalization=None)))  # change
    ...
    vector_map_out = nn.feature_net(inputs['state_vector_map'], time_horizon, **kwargs.get('vector_map', {}), )

    # use RNNs to aggregate predictions over time:
    args = kwargs.get('rnn')
    # image_out = GRU(units=args.get('image'), unroll=True, bias_initializer='glorot_uniform')(stack(image_out))  # change
    # road_out = GRU(units=args.get('road'), unroll=True, bias_initializer='glorot_uniform')(stack(road_out))  # change
    ...
    # change
    vector_map_out = GRU(units=args.get('vector_map'), unroll=True, bias_initializer='glorot_uniform')(
        stack(vector_map_out))

    # intermediate input for control and value networks
    # dynamics_in = concatenate([image_out, road_out, vehicle_out, navigation_out],  # change
    #                           name='dynamics_in')
    dynamics_in = concatenate([vehicle_out, navigation_out, vector_map_out],
                              name='dynamics_in')
...
class CARLANetwork(Network):
    ...
    def data_for_dynamics(self, inputs):
        ...
        # 修复: 检查memory是否为None或actions是否为空
        if memory is None or memory.actions is None:
            inputs['action'] = tf.zeros((1, self.agent.num_actions))
        elif tf.shape(memory.actions)[0] == 0:
            inputs['action'] = tf.zeros((1, self.agent.num_actions))
        else:
