...
class FakeCARLAEnvironment(gym.Env):
    ...
    def __init__(self):
        ...
        # self.NAVIGATION_FEATURES['default'] = np.zeros(shape=self.num_waypoints, dtype=np.float32)  # change

        self.time_horizon = 1
        self.action_space = env.ACTION['space']
        # self.observation_space = gym.spaces.Dict(road=env.ROAD_FEATURES['space'],   # change
        ...
        self.observation_space = gym.spaces.Dict(
            ...
            past_control=env.CONTROL['space'], command=env.COMMAND_SPACE,
            navigation=self.NAVIGATION_FEATURES['space'],
            vector_map=self.VECTOR_MAP_FEATURES['space'])
...
class CARLAgent(PPOAgent):
    ...
    DEFAULT_DYNAMICS = dict(
        # road=dict(units=16, num_layers=2, activation=tf.nn.relu6),  # change
        vehicle=dict(units=16, num_layers=2, activation=tf.nn.relu6),
        navigation=dict(units=16, num_layers=2, activation=tf.nn.relu6),
        vector_map=dict(units=64, num_layers=2, activation=tf.nn.relu6),
        shufflenet=dict(g=1.0, last_channels=768),
        # rnn=dict(image=256, road=32, vehicle=32, navigation=32), # change
        rnn=dict(vehicle=32, navigation=32, vector_map=64),
    ...
    def update(self):
        if len(self.memory) < self.batch_size:
            # Still need to reset info buffer even if update is skipped
            self.env.reset_info()
            return
        ...
        # CARLA 0.9.15 Fix: Use environment's reset_info method (now properly fixed)
        self.env.reset_info()
    ...
    def evaluate(self, name: str, timesteps: int, trials: int, seeds: Union[None, List[int]] = None,
        ...
        results = dict(collision_rate=[],  # similarity=[],   # change
                       waypoint_distance=[],
                       speed=[], total_reward=[], timesteps=[], total_distances=[], total_accelerations=[],
                       total_jerks=[])
        save_path = os.path.join(self.evaluation_path, f'{name}.json')
        save_reward_path = os.path.join(self.evaluation_path, f'{name}_reward.json')
        print(save_path)
        ...
        try:
            ...
            while trials_done < trials:
                ...
                # Step-wise reward log for this trial
                step_reward_log = []

                # vehicle statistics
                # similarity = 0.0  # change
                speed = 0.0
                waypoint_distance = 0.0
                total_distance = 0.0
                acceleration = 0.0
                jerk = 0.0  # 初始化 jerk 累计值
                prev_acceleration = None  # 记录上一个加速度
                ...
                for t in range(1, timesteps + 1):
                    ...
                    for _ in range(self.repeat_action):
                        ...
                        total_reward += reward
                        # 获取当前速度和加速度
                        current_speed = carla_utils.speed(self.env.vehicle)
                        current_acc = carla_utils.acceleration(self.env.vehicle)
                        # 存入完整数据
                        step_reward_log.append((reward, current_speed, current_acc))

                        if done:
                            break

                    with open(save_reward_path, 'w') as f_step:
                        f_step.write("step, reward, speed, acceleration\n")  # CSV 表头
                        for i, (r, s, a) in enumerate(step_reward_log):
                            f_step.write(f"{i + 1}, {r:.4f}, {s:.4f}, {a:.4f}\n")

                    # similarity += self.env.similarity   # change
                    ...
                    acceleration += carla_utils.acceleration(self.env.vehicle)
                    ...
                    if (done or (t == timesteps)) and (t > 32):
                        ...
                        # results['similarity'].append(similarity / t)   # change
                        results['waypoint_distance'].append(waypoint_distance / t)
                        results['speed'].append(speed / t)
                        results['total_distances'].append(total_distance / t)
                        results['total_accelerations'].append(acceleration / t)
                        results['total_jerks'].append(jerk / t)
    ...
    def policy_batch_tensors(self) -> Union[tuple, dict]:
        ...
        # similarity = utils.to_tensor(self.env.info_buffer['similarity'], expand_axis=-1)  # change

        return states, advantages, log_probabilities, speed  # , similarity # change
    ...
    def value_batch_tensors(self) -> Union[tuple, dict]:
        ...
        if speed.shape[0] >= returns.shape[0]:
            ...
            # similarity = similarity[:returns.shape[0]]    # change
        else:
            n = returns.shape[0] - speed.shape[0]
            speed = tf.concat([speed, tf.zeros((n, 1))], axis=0)
            # similarity = tf.concat([similarity, tf.zeros((n, 1))], axis=0)    # change

        return states, returns, speed  # , similarity  # change

    def get_policy_gradients(self, batch):
        states, advantages, log_probabilities, speed = batch  # , similarity = batch  # change

        if self.should_update_dynamics:
            with tf.GradientTape(persistent=True) as tape:
                # dynamics prediction
                dynamics_out = self.network.dynamics_predict_train(states)
                new_batch = (dynamics_out, advantages, log_probabilities, speed)  # , similarity)  # change
    ...
    def policy_objective(self, batch):
        states, advantages, old_log_prob, true_speed = batch  # , true_similarity = batch  # change
        policy = self.policy_predict(states)

        log_prob = policy['log_prob']
        entropy = tf.reduce_mean(policy['entropy'])
        speed = policy['speed']
        # similarity = policy['similarity']  # change
        ...
        # similarity_loss = 0.5 * tf.reduce_mean(losses.MSE(y_true=true_similarity, y_pred=similarity))  # change

        # total loss
        policy_loss = -tf.reduce_mean(tf.minimum(ratio * advantages, min_adv))
        total_loss = policy_loss - entropy_penalty + speed_loss  # + similarity_loss  # change
        ...
        self.log(ratio=tf.reduce_mean(ratio), log_prob=tf.reduce_mean(log_prob), entropy=entropy,
                 ...
                 loss_policy=policy_loss, loss_entropy=entropy_penalty, speed_pi=tf.reduce_mean(speed))  # , change
        # loss_similarity_policy=similarity_loss, similarity_pi=tf.reduce_mean(similarity))

        return total_loss

    def get_value_gradients(self, batch):
        states, returns, speed = batch  # , similarity = batch  # change

        if self.should_update_dynamics:
            with tf.GradientTape(persistent=True) as tape:
                # dynamics prediction
                dynamics_out = self.network.dynamics_predict_train(states)
                new_batch = (dynamics_out, returns, speed)  # , similarity)  # change
    ...
    def value_objective(self, batch):
        states, returns, true_speed = batch  # , true_similarity = batch  # change
        prediction = self.value_predict(states)
        values, speed = prediction['value'], prediction['speed']  # , similarity, prediction['similarity']  # change
        ...
        # similarity_loss = tf.reduce_mean(losses.MSE(y_true=true_similarity, y_pred=similarity))  # change

        self.log(speed_v=tf.reduce_mean(speed),  # similarity_v=tf.reduce_mean(similarity),   # change
                 loss_v=value_loss, loss_speed_value=speed_loss)  # , loss_similarity_value=similarity_loss)  # change

        return (value_loss + speed_loss) * 0.25  # + similarity_loss  # change
    ...
    def augment(self):
        ...
        def prepare(states: list) -> dict:
            # Handle both single observations and time series observations
            if len(states) == 1 and isinstance(states[0], list):
                # Time series case: states = [[obs1, obs2, obs3, obs4]]
                obs_list = states[0]
            else:
                # Single observation case: states = [obs]
                obs_list = states

            # Get keys from first observation
            first_obs = obs_list[0]
            tmp = {k: [] for k in first_obs.keys()}

            for state in obs_list:
            ...
            # Only add state_ prefix if it doesn't already exist
            result = {}
            for k, v in tmp.items():
                if k.startswith('state_'):
                    result[k] = v  # Already has prefix
                else:
                    result[f'state_{k}'] = v  # Add prefix
        ...
        def augment_fn(states: list):
            ...
            # image = utils.to_float(state['state_image'])   # change
